...
Planit.Marker = (function() {
  ...
  Marker.create = function(options) {
	...
	var markerdiv = "<div></div>";
      if (color == "#00008b")
	{   
          markerdiv = "<div><font color='#00008b'>X</font></div>";
      } else if (color == "#8b0000") 
	{
          markerdiv = "<div><font color='#8b0000'>///</font></div>";
	}
    left = ((parseFloat(options.coords[0]) / 100) * container.width()) - 15;
    top = ((parseFloat(options.coords[1]) / 100) * container.height()) - 15;
    markersContainer.append($(markerdiv).addClass(Planit.markerClass).attr({
