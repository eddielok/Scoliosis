...
function AugenmassView(canvas) {
	...
	this.RemoveAllLine = function() {
		...
		this.model_.ResetLine();
	}
	this.GetLength = function() {
		this.model_.GetLength();
	}
	this.startSavedline = function(x,y) {
		help_system.achievementUnlocked(HelpLevelEnum.DONE_START_LINE);
		//this.model_.updatecount();
		this.model_.startEditLine(x, y);
	}
	this.updateSavedline = function(x,y) {	
		//this.model_.updatecount();
		this.model_.updateEditLine(x, y);
	}
	this.confirmSavedline = function() {
		this.model_.commitEditLine();
		help_system.achievementUnlocked(HelpLevelEnum.DONE_FINISH_LINE);
		this.drawAll();
	}
	this.setLabel = function(L) {
		var line = this.model_.getEditLine();
		line.Labels.update(L);
	}
    // Draw all the lines!
	...
	this.SaveLine = function() {
		...
		LineArray = this.model_.lines_;//console.log(LineArray);
		for (var i = 0; i < LineArray.length; ++i) {
			arrlist = arrlist + LineArray[i].Labels.get_label() + "|" + LineArray[i].p1.x + "|" + LineArray[i].p1.y + "|" + LineArray[i].p2.x + "|" + LineArray[i].p2.y  + "_"; 
		}
		var hddline = document.getElementById('hddline');
		hddline.value = arrlist;
		return arrlist;
	};
		this.SaveResizeLine = function() {
		var arrlist = "";
		var LineArray = new Array();
		var ddlresize = document.getElementById('llbzoom');
		var br = document.getElementById('br');
		var ct = document.getElementById('ct');
		//alert(br);alert(ct);
		var offset = (parseFloat(llbzoom.innerHTML.replace('%',''))).toPrecision(4);
		LineArray = this.model_.lines_;//console.log(LineArray);
		for (var i = 0; i < LineArray.length; ++i) {
			arrlist = arrlist + LineArray[i].Labels.get_label() + "|" + LineArray[i].p1.x + "|" + LineArray[i].p1.y + "|" + LineArray[i].p2.x + "|" + LineArray[i].p2.y + "|" + offset + "|" + br.value + "|" + ct.value + "_"; 
		}
		var hddline = document.getElementById('hddline');
		hddline.value = arrlist;
		return arrlist;
	};
	this.DrawAngle = function(x,y) {
		var line = this.model_.getEditLine();
		...
		if (X11.value  > 0 && Y11.value  > 0 && X12.value  > 0 && Y12.value  > 0 != null && X21.value  > 0 && Y21.value  > 0 && X22.value  > 0 && Y22.value  > 0)
		{ 
		var A1x = X11.value;
		var A1y = Y11.value;
		var A2x = X12.value;
		...
		var degree_angle = (angle * (180 / Math.PI)).toPrecision(4)	;
		var Angle = document.getElementById('Angle');
		var PIangle = (180-degree_angle).toPrecision(4);
		if (degree_angle < 90) { Angle.value = degree_angle + '/' + PIangle; } else { Angle.value = PIangle + '/' + degree_angle; }		
		}
	}
	...
	if (pixel_len >= 2) {
	    ...
	    writeLabel(ctx, (print_text*0.026458).toPrecision(4), line.p1.x + text_dx, line.p1.y + text_dy,
		       "center");
	}
    }
	...
	var print_text = (length_factor * line.length()*0.026458).toPrecision(4);
	...
	drawT(ctx, line.p2.x, line.p2.y, line.p1.x, line.p1.y,
	     10);
		drawT(ctx, line.p2.x, line.p2.y, line.p1.y, line.p1.x,
	     10);
	ctx.stroke();
	// .. and text.
	...
	var print_text_label = document.getElementById('ddllabel');
	var Cali = document.getElementById('ddlcal');
	if (Cali.options[Cali.selectedIndex].value == "No")
	{
	print_text = print_text_label.options[print_text_label.selectedIndex].value;
	}
	else {
		print_text = print_text + "cm";
	}
	if (line.Labels.get_label() == "0")
	{
	line.Labels.update(print_text);//alert(line.Labels.get_label());
	} 
	writeLabel(ctx, line.Labels.get_label(),
